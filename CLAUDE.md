...
- **Crontab 回调跳转**：[`CrontabReferences`](src/main/java/com/base/idea/hyperf/crontab/CrontabReferences.java) 匹配 `#[Crontab(callback: "execute")]` 的 callback 字符串（`ParameterList.getParameter("callback", 6)` 覆盖命名/位置参数），跳注解所在类的同名方法，补全列出类内方法名（框架按 `[当前类, callback]` 调用）。
