...
1. **`CompletionContributor`**（注册为 `completion.contributor` EP，`language="any"`）和 **`GotoHandler`**（`gotoDeclarationHandler` EP）是两个入口，分别负责补全和跳转。`CompletionContributor` 覆写了 `invokeAutoPopup`：字符串字面量内输入字母即自动弹补全（无建议时不弹）。
...
- **应用根解析**：[`HyperfRootUtil.resolve(project)`](src/main/java/com/base/idea/hyperf/util/HyperfRootUtil.java) 定位 Hyperf 应用根（BASE_PATH 所在目录），支持 Hyperf 在项目一层子目录的场景（如项目根 bigdata/、应用 bigdata/php/）。定位顺序：`bin/hyperf.php` → 子目录 `*/bin/hyperf.php` → `vendor/hyperf` → 子目录 `*/vendor/hyperf`，按项目缓存、VFS 结构变化自动失效。所有需要"项目根"的功能（BASE_PATH 补全、`.env` 索引、vendor 组件探测、composer.lock、console 执行目录）统一走它，不要直接用 `project.getBaseDir()`。
- **启用开关**：[HyperfStartupActivity](src/main/java/com/base/idea/hyperf/HyperfStartupActivity.java)（`postStartupActivity` EP）在能定位到应用根时弹启用提示；静态方法 `isEnabled(...)` 被所有 References 在匹配前调用，未启用则全部功能短路。
...
- **BASE_PATH 路径补全/跳转**：[`BasePathReferences`](src/main/java/com/base/idea/hyperf/path/BasePathReferences.java) 匹配 `BASE_PATH . '/a/b' . $v` 拼接链中、与 `BASE_PATH` 常量之间只隔字符串字面量的字符串（`matchBasePathConcat` 递归求值左子树，PHP 拼接左结合），以 Hyperf 应用根为基准补全子目录/文件（目录优先、选中目录补 `/` 并 `AutoPopupController` 重弹补全），Ctrl+B 跳到文件/目录。前缀/路径从 `getOriginalPosition()` 真实文本重算，规避补全副本 dummy 占位符污染。
...
- **AOP 切面字符串跳转/补全**：[`AspectReferences`](src/main/java/com/base/idea/hyperf/aop/AspectReferences.java) 匹配 `#[Aspect(...)]` 注解内字符串与 `AbstractAspect` 子类 `$classes/$annotations` 默认值，支持 `'FQN::method'` 整串与 `Foo::class . '::method'` 拼接（`matchClassConstantConcat` 从拼接左子树取 `::class` 的类名）；方法部分支持 `*` 通配（列出全部匹配方法），类部分带 `*` 不跳转。类名已知时（`::` 后 / 拼接字符串内）补全该类方法名。
- **缓存监听器配对跳转**：[`CacheListenerStubIndex`](src/main/java/com/base/idea/hyperf/stub/CacheListenerStubIndex.java) 索引 `#[Cacheable/#[FailCache]` 的 `listener` 参数值与 `new DeleteListenerEvent('name', ...)` 第 1 参（上下文判定收敛在 [`CacheListenerUtil`](src/main/java/com/base/idea/hyperf/cache/CacheListenerUtil.java)，注解参数定位用 [`PsiElementUtils.getParameter`](src/main/java/com/base/idea/hyperf/util/PsiElementUtils.java)`(parameterList, "listener", 3)` 同时覆盖命名/位置参数）；[`CacheListenerReferences`](src/main/java/com/base/idea/hyperf/cache/CacheListenerReferences.java) 实现定向互跳（事件使用侧→注解声明侧，声明侧→全部使用点）与名字补全。
- **DI 接口绑定悬停文档**（不抢占 Ctrl+B）：[`DiBindingStubIndex`](src/main/java/com/base/idea/hyperf/stub/DiBindingStubIndex.java) 索引项目 `config/autoload/dependencies.php` 与任意 `ConfigProvider.php` 的 `dependencies` 子数组，值存 `implFqn|priority|isPD`（[`DiBindingValue`](src/main/java/com/base/idea/hyperf/di/DiBindingValue.java) 解析 `::class` / `new PriorityDefinition(X::class, n)`）；[`DiBindingResolver`](src/main/java/com/base/idea/hyperf/di/DiBindingResolver.java) 按运行时规则选生效实现——项目 dependencies.php 无条件覆盖（`DefinitionSourceFactory` 的 array_replace），vendor 间按 composer.lock 声明顺序模拟 `ProviderConfig::merge`（PD 形态有覆盖保护、权重高者赢、同权重 lock 靠前赢，lock 包序按时间戳缓存）；[`DiBindingDocumentationProvider`](src/main/java/com/base/idea/hyperf/di/DiBindingDocumentationProvider.java)（`lang.documentationProvider`，**order="first"** 先于 PHP 原生被调，内部链式委托拼出原生文档后）在接口悬停文档内容区末尾追加 `Dependencies: \FQN` 可点击链接。
- **Crontab 回调跳转**：[`CrontabReferences`](src/main/java/com/base/idea/hyperf/crontab/CrontabReferences.java) 匹配 `#[Crontab(callback: "execute")]` 的 callback 字符串（`PsiElementUtils.getParameter(parameterList, "callback", 6)` 覆盖命名/位置参数），跳注解所在类的同名方法，补全列出类内方法名（框架按 `[当前类, callback]` 调用）。
...
- `project.getBaseDir()` 可能返回 null，所有调用点需防护；且项目根不一定是 Hyperf 应用根（应用在子目录的场景），需要应用根时用 `HyperfRootUtil.resolve(project)`。
- **`ParameterList.getParameter(String, int)` 是 internal API（Marketplace 验证器禁用）**：用 `PsiElementUtils.getParameter(parameterList, name, index)` 替代（公开 API 重写，命名参数 `name:` 是参数值的前置兄弟节点，跳过空白/注释；单参数 `getParameter(int)` 非 internal，可直接用）。
- **文档弹窗链接只能用 `psi_element://` 协议**：`DocumentationManager.navigateByLink` 硬编码只对该协议走 provider 解析，自定义 scheme 会被当浏览器 URL 报 Malformed URL；且调用 `getDocumentationElementForLink` 时前缀已被平台剥掉，收到的是裸 FQN。
